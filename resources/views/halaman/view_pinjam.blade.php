...
@section('isihalaman')
    ...
    <h3><center>DFA-Library</center></h3>
        ...
        <tbody>
            @foreach ($pinjam as $index=>$p)
                <tr>
                    ...
                    <td align="center">{{$p->id_peminjam}}</td>
                    <td>{{$p->petugas->nama_petugas}}</td>
                    <td>{{$p->anggota->nama_anggota}}</td>
                    <td>{{$p->buku->judul}}</td>
                    <td align="center">
                        <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#modalPinjamEdit{{$p->id_peminjam}}"> 
                            Edit
                        </button>

                        <!-- Awal Modal EDIT data Peminjaman -->
                        <div class="modal fade" id="modalPinjamEdit{{$p->id_peminjam}}" tabindex="-1" role="dialog" aria-labelledby="modalPinjamEditLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form name="formpinjamedit" id="formpinjamedit" action="/pinjam/edit/{{ $p->id_peminjam}} " method="post" enctype="multipart/form-data">
                                            @csrf
                                            {{ method_field('PUT') }}
                                            <div class="form-group row"> 
                                                <label for="id_peminjam" class="col-sm-4 col-form-label">ID Peminjam</label>
                                                <div class="col-sm-8">
                                                    <input type="text" class="form-control" id="id_peminjam" name="id_peminjam" value="{{ $p->id_peminjam}}" readonly>
                                                </div>
                                            </div>
                        ...
                        |
                        <a href="pinjam/hapus/{{$p->id_peminjam}}" onclick="return confirm('Yakin mau dihapus?')">
